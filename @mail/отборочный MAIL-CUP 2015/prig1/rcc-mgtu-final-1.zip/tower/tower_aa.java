...
public class tower_aa implements Runnable {

	public static void main(String[] args) {
		new Thread(new tower_aa()).start();
	}
	...
	class Block implements Comparable<Block> {
		...
		public int compareTo(Block arg0) {
			return (int) Math.signum(-(l - arg0.l));
		}
	...
	void solve() throws IOException {
		...
		for (int i = 0; i < n; i++) {
			square[a[i].color] += 5 * a[i].l * a[i].l;
			square[a[i].color] -= a[i + 1].l * a[i + 1].l;
		}
